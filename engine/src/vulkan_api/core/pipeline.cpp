...
namespace engine
{
        ...
        : Pipeline(device)
    {
        ...
        {
            map_entries.push_back({specialization_constant.first, ToUint32_t(data.size()), specialization_constant.second.size()});
            data.insert(data.end(), specialization_constant.second.begin(), specialization_constant.second.end());
        }

        VkSpecializationInfo specialization_info{};
        specialization_info.mapEntryCount = ToUint32_t(map_entries.size());
        ...
        create_info.stageCount = ToUint32_t(stage_create_infos.size());
        ...
        vertex_input_state.vertexAttributeDescriptionCount = ToUint32_t(pipeline_state.GetVertexInputState().attributes.size());

        vertex_input_state.pVertexBindingDescriptions = pipeline_state.GetVertexInputState().bindings.data();
        vertex_input_state.vertexBindingDescriptionCount = ToUint32_t(pipeline_state.GetVertexInputState().bindings.size());
        ...
        color_blend_state.attachmentCount = ToUint32_t(pipeline_state.GetColorBlendState().attachments.size());
        color_blend_state.pAttachments = reinterpret_cast<const VkPipelineColorBlendAttachmentState *>(pipeline_state.GetColorBlendState().attachments.data());
        ...
        dynamic_state.dynamicStateCount = ToUint32_t(dynamic_states.size());
        ...
        auto result = vkCreateGraphicsPipelines(device.GetHandle(), pipeline_cache, 1, &create_info, nullptr, &m_Handle);
        ...
        : Pipeline(device)
    {
        ...
        result = vkCreateComputePipelines(device.GetHandle(), pipeline_cache, 1, &create_info, nullptr, &m_Handle);
