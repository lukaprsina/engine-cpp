...
# following modification; you may not use this file except in compliance with
# the Apache License and the following modification to it: Section 6.
# Trademarks. is deleted and replaced with:
#
# 1. Trademarks. This License does not grant permission to use the trade names,
#   trademarks, service marks, or product names of the Licensor and its
#   affiliates, except as required to comply with Section 4(c) of the License
...
# Try to find GLFW library and include path. Once done this will define
#
# GLFW_FOUND GLFW_INCLUDE_DIR GLFW_LIBRARIES
#

if(NOT NO_GLFW_X11)
  set(GLFW_X11_INCLUDE_DIRS "/usr/X11R6/include" "/usr/include/X11")
  set(GLFW_X11_LIB_DIRS "/usr/X11R6/lib")
else()
...
find_path(
  GLFW_INCLUDE_DIR
  NAMES GLFW/glfw3.h
  HINTS "${GLFW_LOCATION}/include" "$ENV{GLFW_LOCATION}/include"
...
if(WIN32)
  if(CYGWIN)
    find_library(
      GLFW_glfw_LIBRARY
      NAMES glfw32
      HINTS "${GLFW_LOCATION}/lib" "${GLFW_LOCATION}/lib/x64"
            "$ENV{GLFW_LOCATION}/lib"
      PATHS "${OPENGL_LIBRARY_DIR}" /usr/lib /usr/lib/w32api /usr/local/lib
            "${GLFW_X11_LIB_DIRS}"
      DOC "The GLFW library")
  else()
    find_library(
      GLFW_glfw_LIBRARY
      NAMES glfw32 glfw32s glfw glfw3
      HINTS "${GLFW_LOCATION}/lib"
      ...
      PATHS "$ENV{PROGRAMFILES}/GLFW/lib" "${OPENGL_LIBRARY_DIR}"
      DOC "The GLFW library")
  endif()
else()
  if(APPLE)
    find_library(
      GLFW_glfw_LIBRARY glfw
      NAMES glfw glfw3
      HINTS "${GLFW_LOCATION}/lib" "${GLFW_LOCATION}/lib/cocoa"
            "$ENV{GLFW_LOCATION}/lib" "$ENV{GLFW_LOCATION}/lib/cocoa"
      PATHS /usr/local/lib)
    set(GLFW_cocoa_LIBRARY
        "-framework Cocoa"
        CACHE STRING "Cocoa framework for OSX")
    set(GLFW_corevideo_LIBRARY
        "-framework CoreVideo"
        CACHE STRING "CoreVideo framework for OSX")
    set(GLFW_iokit_LIBRARY
        "-framework IOKit"
    ...
    find_library(
      GLFW_glfw_LIBRARY
      NAMES glfw glfw3
      HINTS "${GLFW_LOCATION}/lib" "$ENV{GLFW_LOCATION}/lib"
...
if(GLFW_INCLUDE_DIR)
  ...
  if(GLFW_glfw_LIBRARY)
    set(GLFW_LIBRARIES
        "${GLFW_glfw_LIBRARY}" "${GLFW_x11_LIBRARY}" "${GLFW_cocoa_LIBRARY}"
        "${GLFW_iokit_LIBRARY}" "${GLFW_corevideo_LIBRARY}")
    set(GLFW_FOUND "YES")
  ...
  if(${GLFW_VERSION_MAJOR}
     OR ${GLFW_VERSION_MINOR}
     OR ${GLFW_VERSION_REVISION})
    set(GLFW_VERSION
...
mark_as_advanced(GLFW_INCLUDE_DIR GLFW_LIBRARIES GLFW_glfw_LIBRARY
                 GLFW_cocoa_LIBRARY)
